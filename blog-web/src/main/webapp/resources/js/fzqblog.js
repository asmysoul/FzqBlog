			...
			$.ajax({
				...
				success: function(data) {
					if (data.errorMsg == null) {
						var d = dialog({
							content: "<div><img src='../resources/images/loading.gif' />&nbsp;&nbsp;&nbsp;注册成功,登录中......</div>",
						});
						d.showModal();
						setTimeout(function() {
							d.close().remove();
							document.location.href = "http://localhost:8091/index";
						}, 1000);
					} else {
						var d = dialog({
							width: 200,
		...
		if (account == null || $.trim(account) == '') {
			...
			$.ajax({
				...
				success: function(data) {
					if (data.errorMsg == null) {
						var d = dialog({
							content: "<div><img src='../resources/images/loading.gif' />&nbsp;&nbsp;&nbsp;登录成功,跳转中...</div>",
						});
						d.showModal();
						setTimeout(function() {
							d.close().remove();
	...
	*/
	...
	$('#findpassword').click(function(event) {
		...
		if (email == null || $.trim(email) == '') {
			...
			var d = dialog({
				content: "<div><img src='../resources/images/loading.gif' />&nbsp;&nbsp;&nbsp;邮箱发送中...</div>",
			});
			d.showModal();
			setTimeout(function() {
				d.close().remove();
								...
								if (password == null || $.trim(password) == '') {
									...
									$.ajax({
										...
										success: function(data) {
											if (data.errorMsg == null) {
												var d = dialog({
													content: "<div><img src='../resources/images/loading.gif' />&nbsp;&nbsp;&nbsp;修改成功,跳转中...</div>",
												});
												d.showModal();
												setTimeout(function() {
													d.close().remove();
...
//控制用户信息显示  加载用户签到信息
	...
	$("#userheadicon").mouseover(function(){
	 ...
	 $("#userheadicon").mouseout(function(event) {
	 	$("#mypanel").hide();
	 });
	 ...
	 $(".unsigninimage").click(function(event) {
			$.ajax({
				...
				success:function(result){
					...
					content: "<div><img src='../resources/images/signin.gif' /></div>",
				});
					d.showModal();
					setTimeout(function() {
						d.close().remove();
