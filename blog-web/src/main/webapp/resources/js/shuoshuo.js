...
var uploader = WebUploader.create({
	auto: true,
	swf: 'localhost:8091/resources/webuploader/Uploader.swf',
...
function ImageItem(img) {
	...
	$('<img src="http://localhost:8091/'  + 'upload/' + img + '" class="showimg">').appendTo(span);
	$('<a href="javascript:void(0)"><img src="http://localhost:8091' + '/resources/images/icon/icon-del.png"></a>').appendTo(span);
	return span;
}
var fzqblog = {};
...
$(document).on("click", ".op-at", function() {
			...
			success:function(res){
				...
				if (_len == 0) {
					...
					for (var i = 0, item; i < _len, item = list[i]; i++) {
						$("<a href='javascript:;' class='at_user'><img src='../resources/images/" + item.friendUserIcon + "' class='img-thumbnail' style='width:40px;height:40px;'><span>" + item.friendUserName + "</span></a>").appendTo(content).click(function() {
							d.close();
	...
	function ShuoShuoItem(data){
		...
		$('<a href="../user/' + data.userId+ '"><img src="../resources/images/'+ data.userIcon + '" class="cd-timeline-img img-circle"></a>').appendTo(item);
		...
		if (data.imageUrlSmall != null && data.imageUrlSmall != ""){
			...
			for (var i = 0, _len = imageSmallArray.length, img; i < _len, img = imageSmallArray[i]; i++){
				if (img !== ""){
					...
					$('<img src="../upload/' + img + '" class="img-thumbnail" style="width: 150px;height: 100px;">').appendTo(userthumbnail);
				}
			}
		}
		...
		var userinfo = $('<a href="http://localhost:8091/user/' + data.userId + '" class="user-info"><i class="glyphicon glyphicon-user"></i><span>' + data.userName + '</span></a>').appendTo(shuoshuoiteminfo);
	...
	function LikeItem(like){
		var likeItem = $('<a href="localhost:8091/user/' + like.userId + '" class="like-info"><img class="user-like-info img-responsive" src="../resources/images/' + like.userIcon + '"></a>');
		return likeItem;
	}
	function CommentItem(comment){
		...
		$('<a href="http://localhost:8091/user/' + comment.userId + '"><img src="../resources/images/' + comment.userIcon + '" class="user-icon-image img-circle"></a>').appendTo(commentusericon);
		var commentmain = $('<div class="comment-main"></a>').appendTo(commentitem);
		var commentcontent = $('<div class="comment-content"><a href="http://localhost:8091/user/' + comment.userId + '">' + comment.userName + '</a>：' + comment.showContent +'</div>').appendTo(commentmain);
